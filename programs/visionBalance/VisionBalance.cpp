...
bool VisionBalance::configure(yarp::os::ResourceFinder &rf) {
    ...
    printf("VisionBalance using cropSelector: %d.\n",cropSelector);
    ...
    printf("VisionBalance using RGBDDevice: %s, RGBDLocal: %s, RGBDRemote: %s.\n",
        strRGBDDevice.c_str(), strRGBDLocal.c_str(), strRGBDRemote.c_str());
    printf("VisionBalance using watchdog: %f.\n",watchdog);
    ...
    if (!rf.check("help")) {
        ...
        printf("[VisionBalance] success: RGBDDevice available.\n");
        if (! dd.view(iRGBDSensor) ) fprintf(stderr,"[ColorRegionDetection] warning: RGBDDevice bad view.\n");
        else printf("[VisionBalance] success: RGBDDevice ok view.\n");
